...
import { getAllCustomers, addNewCustomer, updateCustomer } from '../../actions/customerAction';
...
class CustomerTab extends Component {
    ...
    onUpdateContractor() {
        this.props.updateCustomer(this.state.selectedContractor).then((data) => {
            this.getContractors();
        });
    }
    ...
    addContractor() {
        if (this.state.newContractor.name && this.state.newContractor.email && this.state.newContractor.phone && this.state.newContractor.address) {
            this.state.newContractor.type = 'customer';
            ...
            this.props.addNewCustomer(this.state.newContractor).then((data) => {
                this.getContractors();
            });
        } else {
                                ...
                                return (
                                    <div className="contactor-item" key={index}>
                                        <div className="item-edit-lint" onClick={() => this.onSelectContractor(contactor)}>
                                            <CustomerEditModal index={index} contractor={selectedContractor}
